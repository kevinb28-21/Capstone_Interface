...
Database (PostgreSQL)
  ...
  - imagegps - GPS coordinates
  - telemetry - Current drone state
  - stresszones - Stress zone data
...
Image Upload Flow
  Drone captures image → Uploads to Flask API
  Flask API saves to S3 and creates database record (status: 'uploaded')
  Background worker detects new image
  Worker downloads image from S3 (if needed)
  Worker processes image (NDVI, SAVI, GNDVI)
  Worker saves analysis results to database
  Worker updates status to 'completed'
  Frontend fetches and displays results

Telemetry Flow
  Drone sends telemetry → Node.js API
